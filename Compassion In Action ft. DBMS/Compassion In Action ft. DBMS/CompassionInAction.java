...
class User {
    ...
    public void viewNotifications() {
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            System.out.println("\nNotifications:");
            if (!rs.isBeforeFirst()) { // Check if there are no notifications
                ...
                markNotificationsAsRead(); // Optionally mark all as read after viewing
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
class Admin extends User {
    ...
    public void removeDonation(int index) {
        if (index >= 0 && index < pendingDonations.size()) {
            ...
            // Notif for user donation remove or decline
